...
public class ListImplFailSafe implements List<Product> {
    ...
    class IteratorImpl implements Iterator<Product> {
        ...
        public void remove() {
            ...
            for (int j = currentIndex; j < size - 1; j++) {
                Product temp = snapshot[j];
                snapshot[j] = snapshot[j + INTEGER_ONE];
                snapshot[j + INTEGER_ONE] = temp;
            }
            snapshot[size - INTEGER_ONE] = null;
            size--;
            nextCalled = false;
        }
    }
